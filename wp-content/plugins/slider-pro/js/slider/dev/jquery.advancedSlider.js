	...
	function AdvancedSlider(instance, options) {
		...
		*/
		function create() {
			...
			else if ($.browser.webkit || $.browser.chrome || $.browser.safari)
			...
			if (slides.find('.html .jw-player').length) {
				slides.find('.html .jw-player').each(function() {
					video = $(this);
					jwPlayerArgs = {}
					jwPlayerModes = [];
					flashModeSources = [];
					...
					if (video.data('skin'))
						jwPlayerArgs.skin = video.data('skin');
			...
			if (self.settings.fullscreenControls && !(isIOS || $.browser.msie))
		...
		*/
		function completeTransition() {
			...
			if ($.fn.videoController && (slideDivs[currentIndex].find('.video').length || slideDivs[currentIndex].find('[id^=video]').length)) {
				
				// check if a video instance was already created for the video contained in the current slide
				if (!slidesData[currentIndex].videoInstance) {
					// get the video's HTML element
					var videoElement = slideDivs[currentIndex].find('.video').length ? slideDivs[currentIndex].find('.video') : slideDivs[currentIndex].find('[id^=video]').first();
