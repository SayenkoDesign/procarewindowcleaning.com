...
<section id="updating">
	...
	<h3 id="updating3">3. Updating from a v2 version to the latest v2.3.1</h3>
	...
	<p>As an alternative to uploading all the files, you can upload only those that were modified or added. However, if you want to use this method, please make sure you updated all the mentioned files.</p>
	<p>Files added or modified in V 2.3</p>
	<ul class="custom-list">
	...
	<p>Files added or modified in V 2.3.1</p>
	<ul class="custom-list">
		<li>slider-pro/js/slider/jquery.advancedSlider.min.js</li>
		<li>slider-pro/js/slider/dev/jquery.advancedSlider.js</li>
		<li>slider-pro/css/slider-pro-admin.css</li>
...
<section id="create">
	...
	<h3 id="create8">8. Slide panel - Link & Lightbox</h3>
    <div class="row">
		<div class="span4">
        	<p>The 'Links & Lightbox' area allows you to specify a link or lightbox for both the slide image and the thumbnail image. All you have to do it specify the URL of the link in the 'Path' field. If you want the link to be a lightbox, you will need to check the 'Lightbox' option.</p> 
...
<pre class="prettyprint linenums">
[slider_pro]
    [slide dynamic_posts_types="post;page" dynamic_posts_taxonomies="tag|design;tag|development;tag|planning" dynamic_posts_orderby="comments"]
        [slide_element type="image"][sp_image][/slide_element]
        [slide_element type="link"][sp_link][/slide_element]
        [slide_element type="caption"][sp_content][/slide_element]
    [/slide]
    [slide dynamic_gallery_post="12" dynamic_gallery_maximum="5"]
        [slide_element type="image"][sp_image][/slide_element]
    [/slide]
    [slide dynamic_flickr_api_key="AS23DA423L3698435DDFTU43" dynamic_flickr_data_type="sets" dynamic_flickr_data_id="2343254235"]
        [slide_element type="image"][sp_image][/slide_element]
        [slide_element type="link"][sp_image_set][/slide_element]
    [/slide]
[/slider_pro]
</pre>
		</div>
...
<section id="miscellaneous">
	...
	<div class="row">
    	<div class="span3">
			...
			<p>The slider has built-in support for YouTube videos, Vimeo videos, simple HTML5 videos, HTML5 videos enhanced with VideoJS, and JWPlayer. VideoJS is a 3rd party library that will enhance the look of the HTML5 video and will provide a Flash fallback for browsers that don't support the HTML5 video element, like IE8 and below. JWPlayer offers the possibilities that VideoJS offers and some more, as you can read in the right column.</p>
			<p>The HTML code for the videos must be inserted in the slide's 'Inline HTML' section. In the example code on the right you can see how to insert each type of videos. The videos can also be inserted using a shortcode and the visual editor has a button which can generate the shortcode.</p>
			<p>Please note that in order to have the videos automatically handled you also need go to the 'Videos' sidebar panel and enable the controllers for the videos that you will load.</p>
		</div>
...
&lt;iframe class="video" src="http://www.youtube.com/embed/msIjWthwWwI?rel=0&enablejsapi=1&wmode=transparent" width="580" height="380" frameborder="0"&gt;&lt;/iframe&gt;
			...
			<p>As an alternative to the HTML code above, you can use the following shortcode:</p>
<pre class="prettyprint linenums">
[slider_pro_video type="youtube" id="msIjWthwWwI" params="rel=1&showinfo=0"]
</pre>
...
&lt;iframe class="video" src="http://player.vimeo.com/video/3116167?api=1" width="580" height="380" frameborder="0"&gt;&lt;/iframe&gt;
			...
			<p>As an alternative to the HTML code above, you can use the following shortcode:</p>
<pre class="prettyprint linenums">
[slider_pro_video type="vimeo" id="3116167" params="title=0&byline=0"]
</pre>
...
<pre class="prettyprint linenums">
&lt;video id="video-1" class="video video-js vjs-default-skin" controls="controls" preload="none" 
...
[slider_pro_video type="video-js" 
			...
			<p>JW Player is an advanced video player. In addition to providing both Flash and HTML5 playback, you can use JW Player to play audio files, use different video quality depending on the user's connection speed or stream videos from an RTMP server.</p>
			<p>JW Player is not free and if you want to use it you will first need to download it from <a href="http://www.longtailvideo.com/">http://www.longtailvideo.com/</a>. After you download the package you will need to unzip it, upload it to your server,and point the slider to that folder (which contains the player.swf and jwplayer.js files) by using the slider's "JS Player Path" option, from within the "Videos" sidebar panel.
			<p>You will also need to set the JW Player Controller to true.</p>
			<p>In the example below you can see how to setup the video. You will need to create a main DIV element to which you pass the "video" and "jw-player" classes, and also a unique ID. You will need to use the "data-width" and "data-height" attributes to specify the width of the video, and the "data-image" attribute to specify a poster.</p>
			<p>There are other optional attributes that you can use as well. If you want to set a custom skin, you will use the "data-skin" attribute. You can find more information about using custom skins in JWPlayer <a href="http://www.longtailvideo.com/support/jw-player/jw-player-for-flash-v5/43/using-jw-player-skins">here</a>. If you want to set a provider or a streamer for the player, you will use the "data-provider" and "data-streamer" attributes.</p>
			<p>In order to specify the video files you will need to add some DIV elements, inside the main DIV, with the "source" class. If you want a certain video to be played only in the HTML5 player, you will need to add an additional "html5" class, and if you want the video to be played only in the Flash player, you will need to add the "flash" class. If only the "source" class is added, the player will play the same file in both players.</p>
			<p>A special feature of JW Player is its ability to load different videos depending on the viewer's connection speed. This is a feature supported only in the Flash player, so you will need to add the "flash" class if you intend to use it. As you can see in the example below, you can use the "data-bitrate" attribute to specify the connection speed and the "data-width" attribute to specify the width of the video for each particular version.</p>
			<p>In the example below you can see 6 sources. You don't need so many and most likely you will never use so many but I wanted to illustrate all the possibilities. There are 3 sources for the HTML5 player, so those videos will play only in the browsers that support HTML5 video. The reason why there are 3 videos is because of differences in browser support. For example, Firefox will not support the MP4 video and will need an OGV video instead.</p>
			
<pre class="prettyprint linenums">
&lt;div id="video-0" class="video jw-player" data-width="640" data-height="320" 
     data-image="http://bqworks.com/products/assets/videos/sintel/sintel-poster.jpg"
     data-skin="css/jw-player/glow/glow.zip"&gt;
...
&lt;/div&gt;
			...
			<p>Below are 3 other examples that look more like what you will use in practice:</p>

<pre class="prettyprint linenums">
&lt;div id="video-1" class="video jw-player" data-width="500" data-height="300" 
     data-image="http://bqworks.com/products/assets/videos/sintel/sintel-poster.jpg"&gt;
...
<pre class="prettyprint linenums">
&lt;div id="video-2" class="video jw-player" data-width="500" data-height="300"&gt;
   &lt;div class="source" data-file="http://www.youtube.com/watch?v=FcfWsj9OnsI"&gt;&lt;/div&gt;
&lt;/div&gt;
</pre>

<pre class="prettyprint linenums">
&lt;div id="video-3" class="video jw-player" data-width="500" data-height="300"&gt;
   &lt;div class="source" data-file="path/to/audio.mp3"&gt;&lt;/div&gt;
&lt;/div&gt;
</pre>


			<p>As an alternative to the HTML code above, you can use the following shortcodes:</p>
			
<pre class="prettyprint linenums">
[slider_pro_video type="jw-player"
                  image="http://bqworks.com/products/assets/videos/sintel/sintel-poster.jpg"
                  width="640"
                  height="320"
...
jQuery(document).ready(function($) {
	...
	<div class="row">
		<div class="span7">
			...
			<ul>
				<li>insert the following shortcode in the post/page: [slider_pro_lightbox id="1"]</li>, where the "id" argument holds the ID of the slider instance, as it's displyed in the admin area
				<li>add the "slider-pro-lightbox" class to the element that will open the lightbox. Also, the id of the target element will need to contain the ID of the slider, as you can see in the code block on the right.</li>
			</ul>
			
			<p>If you need a PHP method to load the lightbox slider, you can use &lt;?php slider_pro_lightbox(1);?&gt;</p>
		</div>
		
		<div class="span5">
<pre class="prettyprint linenums">
[slider_pro_lightbox id="1"]

&lt;a id="1" class="slider-pro-lightbox"&gt;Click Me&lt;/a&gt;
</pre>
		</div>
	</div>
