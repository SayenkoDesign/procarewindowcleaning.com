...
define('SLIDER_PRO_VERSION', '2.3.1');
...
function sliderpro_init() {		
	global $sliderpro_slider_skins, $sliderpro_scrollbar_skins, $sliderpro_all_skins, $sliderpro_unique_id, $sliderpro_used_ids, $sliderpro_sliders_js,
		   ...
		   $sliderpro_flickr, $sliderpro_video_unique_id, $sliderpro_lightbox_slider;
...
function slider_pro_video_shortcode($atts, $content = null) {
	...
	if ($atts['type'] == 'youtube' && isset($atts['id'])) {
		...
		$video_markup = '<iframe class="video" src="http://www.youtube.com/embed/' . $atts['id'] . '?enablejsapi=1&wmode=transparent' . $video_params . '"' . 
		...
		$video_markup = '<video class="video" width="' . $width . '" height="' . $height . '" controls="' . $controls . '" preload="' . $preload . '">';
		
		foreach(array('source', 'source1', 'source2', 'source3', 'source4') as $source) {
			if (!isset($atts[$source]))
				continue;
			
			$video_file_extension = substr($atts[$source], -3);
			$video_mime_type = '';
		...
		$video_markup = '<video id="video-' . $sliderpro_video_unique_id . '" class="video video-js vjs-default-skin" data-video="{}" width="' . $width . '" height="' . $height . '" controls="' . $controls . '" preload="' . $preload . '>';
		...
		foreach (array('image', 'skin', 'width', 'height', 'provider', 'streamer') as $att)
			if (isset($atts[$att]))
		...
		foreach(array('source', 'source1', 'source2', 'source3', 'source4', 'source5', 'source6') as $source) {
			if (!isset($atts[$source]))
				continue;
